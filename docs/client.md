...
## Methods
These are some notable and useful methods that are inside Lynk's client framework.
...
This will create a shared module inside the framework, and is how you can create one outside of the framework's structure. Please note that this data will not replicate to the clients, and will only be available in the client. It takes one parameter (`name`, which is a string and represents the name of the shared module), and another optional parameter (`initialData`, which is a table and represents any initial data to be added to the shared module). This function returns a table, which can be used just as any other, and other memebers can be added accordingly.

### `framework:createController(name, initialData)`
This will create a controller inside the framework, and is how you can create a controller outside of the framework's structure. It takes one parameter (`name`, which is a string and represents the name of the controller), and another optional parameter (`initialData`, which is a table and represents any initial data to be added to the controller). This functionr returns a table, which can be used just as any other, and other memebers can be added accordingly.

## Other
Here are a list of some other fields and methods of the client framework. These are here probably because they're used internally for some reason.
